...
func distributeCandies(candies int, num_people int) []int {
	...
	//3.列取余
	yLimit := totalCount % num_people

	//4.计算每一块的内容
	for i := 0; i < num_people; i++ {
		result[i] = (i+1)*xLimit + num_people*xLimit*(xLimit-1)/2
		if i < yLimit {
			result[i] = result[i] + i + 1 + num_people*xLimit
		}
	}
	//5.算上最后一次内容
	result[yLimit] = result[yLimit] + candies - (1+totalCount)*totalCount/2
